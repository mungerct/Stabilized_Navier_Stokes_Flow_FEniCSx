...
def solve_stokes_flow(img_fname, flowrate_ratio, channel_mesh_size, Uold = [], mshOld = [], Vold = []):
    tstart = time.time()
    ...
    W0 = W.sub(0)
    V, _ = W0.collapse()
    noslip = Function(V) # Creating a function makes a function of zeros, which is what is needed for a no-slip BC
    dofs = fem.locate_dofs_topological((W0, V), msh.topology.dim-1, ft.find(4))
    ...
    inlet_1_velocity = fem.Function(V)
    ...
    dofs = fem.locate_dofs_topological((W0, V), msh.topology.dim-1, ft.find(1))
    ...
    inlet_2_velocity = fem.Function(V)
    ...
    dofs = fem.locate_dofs_topological((W0, V), msh.topology.dim-1, ft.find(2))
    ...
    if rank == 0:
        print(f"Finished Combining Boundary Conditions", flush=True)
    ...
    if Uold:
        ...
        interpolation_data = fem.create_interpolation_data(Velocity_Interp.function_space, uold.function_space,
                cells, padding=1.0e-6)

        Velocity_Interp.interpolate_nonmatching(uold, cells, interpolation_data)
        ...
        interpolation_data = fem.create_interpolation_data(Pressure_Interp.function_space, pold.function_space,
                cells, padding=1.0e-6)

        Pressure_Interp.interpolate_nonmatching(pold, cells, interpolation_data)
    ...
    if Uold:
        print('Interpolating Coarse Solution', flush = True)
        U.sub(0).interpolate(Velocity_Interp)
        U.sub(1).interpolate(Pressure_Interp)

    if rank == 0:
        print(f"Starting Linear Solve", flush=True)
    U = problem.solve() # Solve the problem
    log.set_log_level(log.LogLevel.WARNING)

    return U, W, h, msh, tstart, bcs, V, Q

U, W, h, msh, tstart, bcs, V, Q = solve_stokes_flow(img_fname, flowrate_ratio, channel_mesh_size)

# U, W, h, msh, tstart, bcs, V, Q = solve_stokes_flow(img_fname, flowrate_ratio, channel_mesh_size, Ucoarse, mshcoarse, Vcoarse)
